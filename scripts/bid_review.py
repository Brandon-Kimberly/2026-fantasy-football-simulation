...
from fantasy_sim.bid_ledger import MIN_CLAIMS_FOR_A_VERDICT, calibration, load, reconcile, record_bid
...
def main(argv=None):
    ...
    rows = reconcile(load(), _decisions())
    cal = calibration(rows)
    if args.json:
        print(json.dumps({"rows": rows, "calibration": cal}, indent=1, sort_keys=True))
        return cal
